...
import { authenticateToken, AuthRequest, isBuyer, isUpdater } from '../middleware/auth.middleware';
...
const parseAccountId = (id: string): string | null => {
    if (!id) return null;
...
router.post('/accounts', authenticateToken, isBuyer, upload.single('file'), async (req: AuthRequest, res: Response) => {
    try {
        if (!req.file) {
        ...
        const batch = await prisma.accountBatch.findUnique({ where: { id: batchId } });
        if (!batch) {
        ...
        for (let i = 0; i < Math.min(5, data.length); i++) {
            ...
            if (row && row.some((cell: any) =>
                cell?.toString().includes('Tình trạng') ||
        ...
        headers.forEach((header: any, index: number) => {
            ...
            if (h.includes('tình trạng') || h.includes('status')) colMap.status = index;
            if (h.includes('tên tài khoản') || h.includes('account name')) colMap.accountName = index;
            if (h.includes('id') && (h.includes('khách hàng') || h.includes('account'))) colMap.accountId = index;
            if (h.includes('tiền tệ') || h.includes('currency')) colMap.currency = index;
            if (h.includes('chi phí') || h.includes('cost') || h.includes('spend')) colMap.spending = index;
        });
        ...
        for (const row of dataRows) {
            try {
                ...
                if (firstCell.includes('Tổng số') || !firstCell) continue;
        ...
        const totalAccounts = accountCounts.reduce((sum: number, c: { _count: number }) => sum + c._count, 0);
        const liveAccounts = accountCounts.find((c: { status: string }) => c.status === 'ACTIVE')?._count || 0;
...
import parseBatchExcel from '../utils/excelParser';

router.post('/parse-batch', authenticateToken, isBuyer, upload.single('file'), async (req: AuthRequest, res: Response) => {
    try {
        if (!req.file) {
            res.status(400).json({ error: 'No file uploaded' });
            return;
        ...
        const parsed = parseBatchExcel(req.file.buffer);
...
router.post('/create-batch-with-accounts', authenticateToken, isBuyer, async (req: AuthRequest, res: Response) => {
    try {
...
router.post('/spending', authenticateToken, isUpdater, upload.single('file'), async (req: AuthRequest, res: Response) => {
    try {
        if (!req.file) {
            res.status(400).json({ error: 'No file uploaded' });
            return;
        ...
        for (let i = 0; i < Math.min(5, data.length); i++) {
            const row = data[i];
            if (row && row.some((cell: any) => cell?.toString().includes('Tình trạng'))) {
                headerRowIndex = i;
                break;
            }
        }
        ...
        headers.forEach((header: any, index: number) => {
            const h = header?.toString().toLowerCase() || '';
            if (h.includes('id') && h.includes('khách hàng')) colMap.accountId = index;
            if (h.includes('chi phí') || h.includes('cost')) colMap.spending = index;
        });
...
const parseSpendingExcel = (buffer: Buffer) => {
    ...
    const items: { googleAccountId: string; amount: number }[] = [];
...
router.post('/spending/preview', authenticateToken, isUpdater, upload.single('file'), async (req: AuthRequest, res: Response) => {
    try {
        if (!req.file) {
            res.status(400).json({ error: 'No file uploaded' });
            return;
        ...
        if (!mi) {
            batch = await prisma.accountBatch.findFirst({
        ...
        for (const item of parsed.accounts) {
            ...
            if (mi && !existingAccount) continue;
...
router.post('/spending/confirm', authenticateToken, isUpdater, async (req: AuthRequest, res: Response) => {
    try {
        ...
        // Get or verify batch
        let batch = batchId ? await prisma.accountBatch.findUnique({ where: { id: batchId } }) : null;
        ...
        for (const item of data) {
            ...
            const dayRecords = await prisma.spendingRecord.findMany({
                where: { accountId: account.id, spendingDate: date },
            });

            const otherRecords = dayRecords.filter(
                r => r.invoiceMccId !== account.currentMiId || r.customerId !== account.currentMcId
            );
            ...
            const currentRecord = dayRecords.find(
                r => r.invoiceMccId === account.currentMiId && r.customerId === account.currentMcId
            );
            ...
            if (account.currentMcId) {
                const customerTotal = await prisma.spendingRecord.aggregate({
